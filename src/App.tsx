  ...
  const handleConnectAiStudio = async () => {
    if (window.aistudio) {
      try {
        ...
        if (await window.aistudio.hasSelectedApiKey()) await verifyApiKey(process.env.API_KEY);
      ...
      const { epubBlob } = await processEpub(
        ...
        (p) => {
          ...
          if (p.lastZipPathIndex !== undefined && p.lastNodeIndex !== undefined && p.translatedNodes) {
             const res = { filename: file.name, zipPathIndex: p.lastZipPathIndex, nodeIndex: p.lastNodeIndex, translatedNodes: p.translatedNodes, settings: settings };
